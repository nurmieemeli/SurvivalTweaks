...
public final class UntrustCommand implements CommandExecutor {
    ...
    public UntrustCommand(Server server, ProfileRepository profiles, MessageService messages) {
        this.server = server;
    ...
    public boolean onCommand(
        ...
        if (target.getUniqueId().equals(player.getUniqueId())) {
            messages.send(player, "trust.cannot-untrust-self");
            return true;
        }

        String name = target.getName() == null ? arguments[0] : target.getName();
        Profile profile = profiles.load(player.getUniqueId());
        boolean changed = profile.untrustPlayer(target.getUniqueId());
        if (changed) {
            profiles.save(profile);
        }
        messages.send(
                player,
                changed ? "untrust.untrusted" : "untrust.not-trusted",
